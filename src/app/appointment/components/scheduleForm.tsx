...
const ScheduleForm: React.FC<{ visible: boolean; scheduleAppointment: any }> = ({ visible, scheduleAppointment }) => {
  ...
  useEffect(() => {
    if (visible && scheduleAppointment) {
      ...
      const updatedShifts = daysOfWeek.flatMap((_, index) => {
        const dayShifts = formData.filter((shift: Shift) => shift.dayOfWeek === index + 1);
  ...
  const validateShifts = (data: FormValues) => {
    ...
    data.shifts.forEach((shift, index) => {
      ...
      if (endTime.isBefore(startTime)) {
        setError(`shifts.${index}.endTime`, { message: "End time must be after start time" });
        isValid = false;
      } else {
        clearErrors(`shifts.${index}.endTime`);
      }

      // Ensure shifts do not overlap
      const sameDayShifts = data.shifts.filter((s) => s.dayOfWeek === shift.dayOfWeek);
      if (sameDayShifts.length === 2 && startTime.isBefore(dayjs(sameDayShifts[1].endTime))) {
  ...
  const removeSecondShift = (dayOfWeek: number) => {
    const secondShiftIndex = fields.findIndex((field) => field.dayOfWeek === dayOfWeek && field.shiftNumber === 2);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full mt-6">
      {daysOfWeek.map((day, index) => (
        <div key={index} className="mb-4">
          <Flex align="center" justify="start" className="pb-3 border-b-[1px]">
              ...
              {fields
                ...
                .map((field, fieldIndex) => (
                  <Space key={field.id} align="center">
                    ...
                    />
                    {field.shiftNumber === 2 && (
                      <Button type="dashed" danger onClick={() => removeSecondShift(index + 1)}>
                        Xóa
                      </Button>
                    )}
                  </Space>
            ...
            {fields.filter((field) => field.dayOfWeek === index + 1).length < 2 && (
              <Button
                type="primary"
                onClick={() => append({ dayOfWeek: index + 1, shiftNumber: 2, startTime: "", endTime: "" })}
              >
                Thêm ca thứ 2
              </Button>
            )}
      ...
      <Button type="primary" htmlType="submit" disabled={!formState.isValid} className="mt-4">
        Đặt lịch làm việc
      </Button>
    </form>
  );
